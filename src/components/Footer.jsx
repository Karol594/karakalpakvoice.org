...
// Примечание: В реальном проекте убедитесь, что библиотека react-i18next установлена.
// В данной среде предпросмотра мы будем использовать имитацию i18n для демонстрации.
import { 
  MapPin, Mail, ShieldCheck, Globe, 
  Facebook, Send, Instagram, Youtube, Twitter, Link as LinkIcon, Check,
  X, Bell, ChevronRight, RefreshCcw
} from 'lucide-react';

/**
 * Имитация хука useTranslation для обеспечения работоспособности в среде предпросмотра.
 * В вашем реальном проекте используйте: import { useTranslation } from 'react-i18next';
 */
const useTranslation = () => {
  return {
    i18n: {
      language: 'KK',
      on: (event, callback) => {
        // Имитация прослушивания события смены языка
...
const SubscribeModal = ({ isOpen, onClose, t, lang }) => {
  ...
  // 2. ТЕКСЕРУДІ ОСЫ ЖЕРГЕ (return-нен сәл жоғары) ҚОЯМЫЗ
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
    {/* ... әрі қарай код сол күйінде қалады ... */}
      ...
      <div className="relative bg-[#121212] border border-gray-700 rounded-2xl w-full max-w-md p-8 shadow-2xl text-white">
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-white bg-gray-800/50 p-1 rounded-full"><X size={20}/></button>
        <div className="text-center mb-6">
          <div className="w-12 h-12 bg-blue-600/20 text-blue-500 rounded-full flex items-center justify-center mx-auto mb-4"><Bell size={24}/></div>
          <h3 className="text-xl font-bold mb-2">{t.subscribeBtn}</h3>
          <p className="text-sm text-gray-400">{t.subscribeDesc}</p>
        </div>
        <form className="space-y-4" onSubmit={(e)=>e.preventDefault()}>
          <input type="email" placeholder="Email" className="w-full bg-[#0a0a0a] border border-gray-700 rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500"/>
          {/* --- CAPTCHA БЛОГЫ --- */}
          <div className="bg-[#0a0a0a] p-4 rounded-lg border border-gray-800 mt-4">
            ...
            <div className="flex items-center gap-3">
              {/* Есеп (Мысалы: 5 + 3 = ?) */}
              <div className="flex-1 bg-gray-800/50 rounded px-3 py-2 text-center font-mono text-lg font-bold text-blue-400 tracking-widest border border-gray-700 select-none">
                {num1} + {num2} = ?
              </div>
                  ...
                  <input 
                    ...
                    required
                    className="w-20 bg-white/10 border border-gray-700 rounded-lg px-3 py-2 text-center text-white focus:outline-none focus:border-blue-500 font-bold"
                  />
                  
                  {/* Жаңарту түймесі */}
                  <button type="button" onClick={generateCaptcha} className="p-2 text-gray-500 hover:text-white" title="Жаңалаў">
                    <RefreshCcw size={18} />
                  </button>
                </div>
                
                {/* Қате шықса көрсететін жазу */}
                {error && <p className="text-red-500 text-xs mt-2 font-medium">{error}</p>}
              </div>
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 font-bold py-3 rounded-lg flex items-center justify-center gap-2">
            <span>{t.subscribeAction}</span><Send size={16}/>
          </button>
        </form>
      </div>
    </div>
...
export default function Footer() {
  ...
  // 1. ТҮЗЕТУ: Бастапқы мәнді localStorage-дан немесе i18n-нен аламыз
  ...
  // 2. ТҮЗЕТУ: Тіл өзгеруін тыңдайтын әмбебап Listener
  useEffect(() => {
    // A. i18next кітапханасының өз сигналы
    const handleI18nChange = (lng) => {
      setCurrentLang(lng);
    };

    // B. Navbar-дан келетін "Custom Event" сигналы (Home.jsx-тегідей)
  ...
  // 3. ТҮЗЕТУ: Тіл кілттерін дұрыс нормализациялау (KAA -> KK, кіші әріптерді үлкенге)
  ...
  return (
    <>
      <footer className="relative bg-[#0a0a0a] text-gray-300 font-sans border-t border-gray-800/50">
        ...
        {/* Каракалпакский нақыш (орнамент) — Сделан более ярким и четким */}
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1.5fr] gap-12">
          ...
          <div>
            ...
            <ul className="space-y-3 text-[13px]">
              {content.projectsList.map((p,i)=>(<li key={i}><Link to={p.path} className="hover:text-blue-400 transition">• {p.name}</Link></li>))}
            </ul>
          </div>
