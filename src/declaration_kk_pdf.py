...
# ── Жоллар ───────────────────────────────────────────────────
PHOTO    = r"public\images\sovereignty\resolution (1).jpg"
OUTPUT   = r"public\documents\declaration_kk.pdf"
...
# ── Колонтитул (Конституция менен бирдей стиль) ─────────────────
...
def build_photo_page():
    ...
    if os.path.exists(PHOTO):
        # Фотоны бетке сыйдырыў (алтын жийек ушын орын қалдырамыз)
        img_w = PAGE_W - 2 * MARGIN
        img_h = PAGE_H - 2 * MARGIN - 4.5 * cm  # астынғы текст ушын орын
...
def build_content():
    ...
    s.append(art(8,
        "Қарақалпақстан Республикасы өзиниң пуқаралығына ийе болады, Республика пухаралары "
        "соның менен бир ўақытта Өзбекстан ССРның ҳəм СССРдың пухаралары болып табылады."))

    s.append(art(9,
        "Қарақалпақстан Республикасы өзиниң мəмлекетлик символикасын, гербин, "
...
def build_pdf():
    ...
    print("    Беттер: 1 (фото) + мазмун + жуўмақлаў")
